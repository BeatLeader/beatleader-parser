...
{
    // yoinked from https://github.com/AllPoland/ArcViewer/blob/eab5c898b797d3ff4645a760abda3a485efaa48d/Assets/__Scripts/Utilities/BeatSaberEasings.cs (with permission)
    public class BeatSaberEasings
    {
        public static double BeatSaberInOutBack(double t)
        {
            if (t < 0.517f)
            {
                return 5.014f * t * t * t;
            ...
            {
                double a = Math.Pow(1.665f * (t - 0.4f) - 1f, 3f);
                double b = Math.Pow(1.665f * (t - 0.4f) - 1f, 2f);
                return 1f + (2.70158f * a) + (1.70158f * b);
            }
        }


        public static double BeatSaberInOutElastic(double t)
        {
            if (t < 0.3f)
            {
            ...
            {
                double pow = Math.Pow(2f, -10f * (t - 0.2f));
                double sin = Math.Sin(t * 10f * 2.0943952f);
                return pow * sin + 1f;
            }
        }


        public static double BeatSaberInOutBounce(double t)
        {
            if (t < 0.36363637f)
            {
        ...
        public static double Ease(double t, BeatSaberEasingType easing)
        {
            switch (easing)
            {
                default:
